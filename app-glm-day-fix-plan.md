...
> **v3 变更**：按执行顺序重排——**第一部分格式修复（先做）→ 第二部分质量筛选（后做）→ 第三部分交付**。每个问题先在归属表（§1.2）定位置，正文按层分组，不再按问题编号混排。
>
...
| # | 问题 | 归属 | 所在章节 |
|---|---|---|---|
| P0-1 | chat 源缺 assistant 回复 | **格式** | §3 |
| P0-2 | responses 源缺 assistant 回复 | **格式** | §4 |
| — | 续话客户端（previous_response_id）碎片 | **格式层丢弃** | §4（UDF 规则 4）+ §7 |
| P1-3 | reasoning_effort 值域非法 | **格式** | §5 |
| P1-4a | tools 表达成 `[]` | **格式** | §6 |
| P1-4b | 无工具纯对话流量（99.4%） | **筛选** | §10 |
| P1-5 | 空 messages（6 条） | **格式层丢弃** | §7 |
| — | session 结束在 tool_call 无结果（未闭环） | **筛选** | §9 淘汰清单 + §11 |
| — | 最后请求失败 | **格式**（取最后有效交换）；全失败 → 格式层丢弃 | §3 修法① |
| P2-6 | 无 18 条款筛选 | **筛选** | §11 |
| 附-7 | 顶层 9 列 vs 训练 4 键 | **交付层** | §13 |
