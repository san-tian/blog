...
现状 ❌: MAX_BY(req_messages, ts) → ts=300 的全量历史（第 1+2 轮 + user₃——历史不缺）
        ...
        ⚠ 朴素修法的坑：若只在 INSERT 层防御（resp=NULL 丢整条）而漏掉 CTE 行级过滤——
          连完全正常的前 2 轮也一起丢，一次失败连累整个 session
...
>
> **为什么前两轮天然在、第三轮整轮丢**：chat completions 协议客户端每次重发**全量历史**——ts=200 那行的 `req_messages` 本来就装着第 1+2 轮完整对话，所以取到它 = 前两轮全收。失败的第 3 轮（含它的 user message）整体丢弃：它没得到应答，保留就是"有问无答"的尾巴（不闭环）；数据仍在 dwd，需要时可查。
