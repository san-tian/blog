...
13. **术语跨文章复用**：基础知识先查术语索引 `glossary.md`——讲过的术语直接引用出处（链接原文），不重复解释；没讲过的在文章里单出一页仔细讲（直接定义，不用比喻），发布后登记进术语索引（术语 → 本文位置）。全站同一概念沿用同一术语，不换说法。
...
7. **零基础假设，但用专业术语**：假设读者没有本领域背景——解决方式不是白话或比喻，而是：用该领域常用的标准术语，首次出现（全站维度，先查 `glossary.md`：讲过的直接引用、不重复解释，见写作流程第 13 条）紧跟一句直接定义（是什么、干什么用，一个从句说清），全文名词统一（同一概念只用一个词，不换说法、不用储物柜/邮编/查字典这类比喻）。例：「indexer（索引器）：稀疏注意力里对全部历史 KV 打分、选出 top-k 条目的选路模块」。**未讲过的术语每个单独一页**（每页一个术语：是什么 / 怎么做 / 与本文关系，≤4 卡片）；名词约定（slot、loc、rank 等）放脚注；「乱码」「水位」这类习惯用语也给直接定义。
