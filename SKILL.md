...
**上报规则**：最后向用户上报（交付结果/答复里贴链接）时，一律用公网链接 `http://106.52.237.179:21688/<文件名>.html`，不要用仓库链接（`:3000`，打开的是源码且需登录）或内网链接（`:8081`，仅 tailscale 可达）。「上报」指告诉用户的链接；git 提交/push 照常推仓库，与本规则无关。
