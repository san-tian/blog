...
术语引用约定：只对首次出现的术语详细解释，讲过的直接引用（术语索引见 [glossary.md](glossary.md)）。**DSA（DeepSeek Sparse Attention，稀疏注意力架构）**——GLM-5.2 采用的注意力结构：dense 注意力（MHA/MLA）每步对全部历史 token 计算，长上下文下计算量不可控；DSA 改为两段式：先选路、再计算。**indexer（索引器）**——DSA 的选路模块：对每个 query 用低精度（fp8）打分器给全部历史 KV 打分、选出 top-k 条（GLM-5.2 约 2048），attention 只对选中条目计算；indexer 打分与 attention 计算读同一份 K（即本文的 "indexer KV share"）。这两个术语为本文首次详细解释，已登记术语索引。KV cache、kernel、MLA 旧文已讲过，本文直接使用：prefill/decode/PD 分离见《PD 分离架构里 decode 二级 KV 缓存机制》；MLA 见《在 H200 上服务 GLM-5.2》；DCP 概念（KV 按序列维切分到多卡）见《DCP 实测》，本文只讲 sglang 实现的「全局 slot 编号 → 本卡物理行号」换算；656B 行布局与量化公式见姊妹篇《GLM-5.2 decode TPOT 优化》。另：「乱码」指模型输出变成无意义文字；slot / 行 = KV cache 的一个 token 槽位，loc = 写入目标 slot 编号，rank = 一个 DCP 参与方（一张 GPU 一个 rank）。
