...
4. **漏斗式叙事 + 页型骨架**（10–15 页）：封面（一句话结论 + 排查路径预览）→ 背景（先术语定义页，再技术页）→ 现象（观察组合 + 读出的线索）→ 排查路径总览（漏斗图，每层一个问题）→ 逐层收敛定位（①什么变了 → ②数据/契约长什么样 → ③正确写法是什么 → ④新代码做了吗 → ⑤错误落在哪/何时，每页顶部带排查进度条）→ 修复 → 证据索引（卡片网格）→ 三句话总结。读者能跟着漏斗一层层知道「问题发生在哪」——背景、现象、逐层排除、根因、位置，不要跳步。
...
7. **零基础假设，但用专业术语**：假设读者没有本领域背景——解决方式不是白话或比喻，而是：用该领域常用的标准术语，首次出现（全站维度，先查 [`glossary.md`](glossary.md)：讲过的直接引用、不重复解释，见写作流程第 11 条）紧跟一句直接定义（是什么、干什么用，一个从句说清），全文名词统一（同一概念只用一个词，不换说法、不用储物柜/邮编/查字典这类比喻）。例：「indexer（索引器）：稀疏注意力里对全部历史 KV 打分、选出 top-k 条目的选路模块」。背景放一页术语定义卡（未讲过的术语）＋一条名词约定（slot、loc、rank）；「乱码」「水位」这类习惯用语也给直接定义。
