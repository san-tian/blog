...
| `glossary.md` | 根目录 | 术语索引：跨文章术语知识库，写作前先查（讲过的直接引用，没讲过的详讲后登记） |
...
**上报规则：最后向用户上报（交付结果/答复里贴链接）时，一律给公网链接 `http://106.52.237.179:21688/`，不要给仓库链接（`:3000`）或内网链接（`:8081`）**。「上报」指**告诉用户的那个链接**——git 提交/push 照常推仓库，与本规则无关。仓库链接打开的是源码且需 Gitea 登录，内网链接仅 tailscale 网络可达，公网链接直接打开渲染后的文章。上报具体文章用 `http://106.52.237.179:21688/<文件名>.html`（例：`http://106.52.237.179:21688/app-glm-day-fix-plan.html`）。
...
10. **引用代码附 GitHub 超链接**：引用代码/commit 时尽可能给可点击的链接——commit 用 `https://github.com/<org>/<repo>/commit/<sha>`，文件:行号用 `https://github.com/<org>/<repo>/blob/<sha>/<path>#L起-L止`，**锚定 commit SHA、不锚分支名**（分支会漂移，SHA 永远指向当时的代码）。所在 fork 用实际仓库（如 `MindLab-Research/sglang`、`san-tian/sglang`），上游用 `sglang-org/sglang`；未推送的本地 commit 链不了，标注「本地仓库」即可。
11. **术语跨文章复用**：基础知识先查术语索引 [`glossary.md`](glossary.md)——讲过的术语直接引用出处（链接原文），不重复解释；没讲过的在首次提到处就地加小卡片解释（直接定义，不用比喻），发布后登记进术语索引（术语 → 本文位置）。全站同一概念沿用同一术语，不换说法。
...
- **术语一致**：同一个概念全篇用同一个词，首次出现时括号注明英文原文，之后固定不改。例如「KV cache（键值缓存）」定下后就别再写「KV 缓存 / KV」；「实例 / 副本 / pod / worker」「命中 / 重叠 / 亲和」这类同义混用会徒增读者认知负担——定一个，全文沿用
- **图多字少**：big pictures, few words——能用一张图、一个表格、一段类比讲清的概念，别堆成几段文字；默认读者只有很基础（相当少）的背景知识，机制先给直觉和结构（图示优先），术语首次出现必给直接定义（读者可能连 indexer 是什么都不知道——一句直接解释讲清，不用比喻；详见「排查/讲解类文章」第 7 条），最后再谈细节
