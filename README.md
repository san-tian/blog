...
7. **零基础假设，但用专业术语**：假设读者没有本领域背景——解决方式不是白话或比喻，而是：用该领域常用的标准术语，首次出现紧跟一句直接定义（是什么、干什么用，一个从句说清），全文名词统一（同一概念只用一个词，不换说法、不用储物柜/邮编/查字典这类比喻）。例：「indexer（索引器）：稀疏注意力里对全部历史 KV 打分、选出 top-k 条目的选路模块」。背景放一页术语定义卡（KV cache / indexer / DCP / kernel 等）＋一条名词约定（slot、loc、rank）；「乱码」「水位」这类习惯用语也给直接定义。
