...
| DCP（概念） | KV Cache 切分维度从头维改为序列维，消除高 TP 下 KV 重复存储 | dynamic-context-parallelism.md |
| DCP 上限公式 | DCP 上限 = TP 并行度 ÷ KV 头数（且需整除） | glm52-dcp-config-draft.md |
| DCP（sglang 实现：虚拟 id 换算） | loc 为全局虚拟 slot 编号；写入前换算：归属 rank = v % dcp，物理行号 = v // dcp | dsa-fused-kv-store-dcp-regression.html（P7）；prefill CP 与 decode CP 对比见 why-prefill-cp-breaks-standalone-decode.md（§4.3） |
