...
六项能力中,**MLA 和 Wide-EP 是物理基础** —— 没有它们,仓库级上下文根本装不下,其他优化也无从谈起。

核心矛盾在于:**GLM-5.2 的 MLA 把 KV 压缩到 44 KB/token(对比标准 MHA 的 ~2.5 MB/token),但也让 KV 无法被张量并行(TP)分片**。传统 TP=8 不再"分摊"KV,而是**复制 8 份**,单节点总物理 KV 约 200 GiB,其中绝大部分是冗余。

llm-d 的解法是 **Wide-EP(宽专家并行)**:用**数据并行注意力**(让 KV 在各 rank 本地独立)+ **专家并行 MoE**(把专家权重分散到各 rank),既省权重显存、又避开 TP 的 all-reduce。实测 KV 容量提升:

| 拓扑 | 单节点 KV 容量 |
| --- | --- |
| 聚合 TP=8 | 598K token(200 GiB 物理,冗余占多数) |
| DEP8 | 1.77M token |
| **DEP16** | **约 5.2M token(节点份额)** |

这套组合让每个 GPU 能在 HBM 里塞下约 1M token KV 缓存,为高并发智能体服务提供物理可行性。后续章节会深入拆解这两项技术的工作机制。

## 深度解析:MLA 与 Wide-EP 是怎么做的

前面给出了"MLA 压缩 KV"和"Wide-EP 替代 TP"的结论,但**为什么** MLA 能压、**为什么** TP 不再适用、Wide-EP **怎么** 换掉 TP —— 这些机制细节决定了你能否复现这套部署,或者在其他模型上应用类似思路。

本节的因果链:**MLA 把 KV 压到一个完整的潜在向量 → 张量并行无法沿 head 分片 → TP 变成复制 8 份 → Wide-EP 用 DP attention + EP MoE 替代 → 每 GPU 持不同专家、本地 KV 不复制 → 容量提升数倍**。下面逐层拆解。
...
<details>
<summary><strong>📊 图示:三种方案的 KV 缓存结构对比</strong></summary>

```
┌─────────────────────────────────────────────────────────────┐
│  MHA (Multi-Head Attention) - 每层 32 KB                    │
├─────────────────────────────────────────────────────────────┤
│  ┌───┬───┬───┬───┬───┬───┬───┬───┬───┬───┬───┬───┬───┐     │
│  │K₀ │K₁ │K₂ │...│K₆₃│V₀ │V₁ │V₂ │...│V₆₃│           │  64个KV头  │
│  └───┴───┴───┴───┴───┴───┴───┴───┴───┴───┘           │  各128维   │
│  每个头独立存储,无共享                                 │  → 2.5 MB  │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│  GQA (Grouped Query Attention) - 每层 4 KB                  │
├─────────────────────────────────────────────────────────────┤
│  ┌───┬───┬───┬───┬───┬───┬───┬───┐                         │
│  │K₀ │K₁ │...│K₇ │V₀ │V₁ │...│V₇ │      8组共享           │
│  └───┴───┴───┴───┴───┴───┴───┴───┘      每组服务8个Q头    │
...
┌─────────────────────────────────────────────────────────────┐
│  MLA (Multi-head Latent Attention) - 每层 576 B            │
├─────────────────────────────────────────────────────────────┤
│  ┌──────────────────────┬──────────┐                        │
│  │   c_t (潜在向量)      │  RoPE    │   压缩表征              │
│  │      512 元素         │ 64 元素  │   所有头共享            │
│  └──────────────────────┴──────────┘   → 44 KB             │
│          ↓ 推理时低秩投影                                    │
│     W_K·c_t → K₀..K₆₃, W_V·c_t → V₀..V₆₃                   │
└─────────────────────────────────────────────────────────────┘
```

**关键差异**:
- **MHA**:每个头独立存 K 和 V,灵活但冗余大
- **GQA**:多个 Q 头共享一组 KV,减少存储但仍按头分开
- **MLA**:所有头共享一个压缩的潜在向量,存储最小但**无法沿头分片**
...
<details>
<summary><strong>📊 图示:TP=8 为什么会复制 KV</strong></summary>

```
MHA/GQA 下的 TP=8 (可分片):
┌────────────────────────────────────────────────────────┐
│64 KV 头按 head 维度分片给 8 个 rank                    │
├────────────────────────────────────────────────────────┤
│ Rank0: K₀..K₇ + V₀..V₇   (8 heads, 1/8 KV)                │
│ Rank1: K₈..K₁₅ + V₈..V₁₅  (8 heads, 1/8 KV)                │
│  ...                                                      │
│ Rank7: K₅₆..K₆₃ + V₅₆..V₆₃ (8 heads, 1/8 KV)                │
│                                                              │
│ ✅ 每个 rank 只持有自己那份 KV,总量 = 1×                    │
└────────────────────────────────────────────────────────┘

MLA 下的 TP=8 (不可分片):
┌────────────────────────────────────────────────────────┐
│c_t 是一个不可分的完整向量 (512 元素)                 │
├────────────────────────────────────────────────────────┤
│ Rank0: c_t[0..511] + rope[0..63]  (完整 KV)              │
│ Rank1: c_t[0..511] + rope[0..63]  (完整 KV, 复制!)         │
│  ...                                                      │
│ Rank7: c_t[0..511] + rope[0..63]  (完整 KV, 复制!)         │
│                                                              │
│ ❌ 每个 rank 必须持有完整 c_t,总量 = 8×                    │
│ ❌ 单节点物理 KV ~200 GiB (大部分是冗余副本)           │
└────────────────────────────────────────────────────────┘
```

**为什么不能分片**:c_t 是所有 64 个头的压缩表征,任何一个头都需要完整的 c_t 才能还原出 K/V。把 c_t 按元素切分给不同 rank 会破坏这个表征的语义。
...
#### 为什么需要 Wide-EP:从模型权重说起

要理解 Wide-EP 的作用,先看 GLM-5.2 一个 GPU 上显存里都有什么:

| 组件 | 单 GPU 占用(Wide-EP / DEP16) | 占比 |
| --- | --- | --- |
| 专家权重(Expert Weights) | ~65 GiB | ~57% |
| 共享部分(MLA 注意力 + 路由 + LN + 其他) | ~50 GiB | ~43% |
| **模型权重合计** | **~115 GiB** | 100% |

**专家权重是绝对大头** —— 占了一半以上。而且这部分的容量在 TP=8 下还会继续增长:每个 rank 要承担 ~89.5 GiB 的专家权重(因为全部专家都需要存一份),挤压本就不宽裕的 KV 空间。

更糟的是,**专家权重不会被分摊** —— TP=8 把 MLA 注意力按头分给 8 个 rank,但所有专家都还在每个 rank 上(因为每个 token 都要访问任意专家)。结果就是:

- 注意力部分:TP=8 能省 7/8 的注意力显存
- MoE 部分:TP=8 完全没省 —— 所有 rank 都得持有完整专家集合

也就是说,**TP=8 优化的是占 43% 的那部分(共享部分),而占 57% 的专家部分纹丝不动**。

Wide-EP 的解法是**让专家部分也参与并行**:把 256 个专家分散到 16 个 rank 上,每个 rank 只持有 16 个(约 65 GiB / 16 ≈ 4 GiB)。这部分省下的显存可以全部留给 KV。
...
<details>
<summary><strong>📊 图示:Wide-EP 单层 forward 数据流</strong></summary>

```
DEP16 单层工作流 (数据并行注意力 + 专家并行 MoE):

输入 tokens ──────────┐
                      │
                      ▼
        ┌─────────────────────────┐
        │  1. MLA Attention (DP)  │  ← 每个 rank 处理不同 batch
        │     每 rank 独立计算    │     KV 写入本地 HBM
        └─────────────┬───────────┘
                      │ attn_out
                      ▼
        ┌─────────────────────────┐
        │  2. MoE Gate            │  为每个 token 选 top-k 专家
        │     routing = gate(x)   │  routing: [token → expert_id]
        └─────────────┬───────────┘
                      │
                      ▼
        ┌─────────────────────────┐
        │  3. DeepEP all-to-all   │  把 token 派发到持有对应
        │     dispatch tokens     │  专家的 rank (跨节点 IB)
        └─────────────┬───────────┘
                      │
         ┌────────────┼────────────┐
         │            │            │
         ▼            ▼            ▼
    ┌────────┐  ┌────────┐  ┌────────┐
    │Expert₀ │  │Expert₁ │  │Expert₁₅│  ← 16个rank各持16个专家
    │ Rank0  │  │ Rank1  │  │ Rank15 │    权重分散,省显存
    └────┬───┘  └────┬───┘  └────┬───┘
         │            │            │
         └────────────┼────────────┘
                      │
                      ▼
        ┌─────────────────────────┐
        │  4. DeepEP all-to-all   │  把计算结果送回原 rank
        │     combine results     │
        └─────────────┬───────────┘
                      │
                      ▼
...
关键特性:
• 注意力: 0 次 all-reduce (DP,不是 TP)
• MoE: 1 次 all-to-all dispatch + 1 次 combine
• KV: 本地独立,不跨 rank 复制
• 权重: 65 GiB/GPU (vs TP=8 的 89.5 GiB)
```
...
1. **智能体负载的画像**:98.6% 的 token 在输入侧,中位复用率 99.6%,中位会话 107 轮、持续 1 小时 —— 这要求服务系统在**长 KV 缓存**和**高命中率前缀复用**上做到极致。

2. **MLA 的双刃剑**:把 KV 从 2.5 MB/token 压到 44 KB/token(~57×),让长上下文在物理上可行;但代价是 KV 变成一个**不可分片的潜在向量**,TP 从"分摊"变成"复制"。

3. **Wide-EP 的换血**:用 DP attention(各 rank 本地 KV)+ EP MoE(专家分散持有)替代 TP,把冗余的 8 份 KV 副本变成 8 倍的有效容量,同时省下约 25 GiB/GPU 的专家权重空间。

4. **容量的复利**:DEP16 单 rank 持约 1M token KV;配合前缀路由(复用率 90–92%)+ CPU 卸载(恢复比重算快 2.8×)+ MTP(并发扩展 8×),最终让 64 GPU 在并发 128 时还能维持中位 TTFT 2.18 s、ITL 19.1 ms。

这条链路上的每个环节都是**必要且充分**:少了 MLA,KV 装不下;少了 Wide-EP,MLA 的压缩被 TP 的复制抵消;少了前缀路由和 CPU 卸载,容量再大也会被冷启动和淘汰拖垮。
